...
if __name__ == '__main__':
    ...
    classes    = ["_background_","BS", "NL", "hispa2", "hispa3", "hispa4", "hispa5", "B", "DM", "BLB", "BLS","H","T"]
    ...
    for i in range(0, len(count)):
        ...
        if os.path.isfile(path) and path.endswith('json'):
            ...
            # Check if imageData exists; if not, find the image file on disk
            if 'imageData' in data and data['imageData'] is not None:
                imageData = data['imageData']
            else:
                # Get the image path from the JSON and join it with the directory of the JSON file
                imagePath = os.path.join(os.path.dirname(path), data['imagePath'])
                if not os.path.exists(imagePath):
                    # If the path in JSON is wrong, assume the image is in the same folder as the JSON
                    imagePath = os.path.join(os.path.dirname(path), count[i].replace(".json", ".jpg"))
